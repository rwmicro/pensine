...
> [!tip] Modèle mental : journald, un journal binaire indexé
> ```
...
> [!important] La grande question : volatile ou persistant ?
> Par défaut, si `/var/log/journal/` n'existe pas, journald écrit dans `/run/log/journal/` — en **RAM**, donc **perdu au reboot**. Pour conserver les logs entre redémarrages :
> ```
