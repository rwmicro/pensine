...
> [!tip] Modèle mental : les trois agents du mail
> ```
...
> [!warning] L'étape qu'on oublie : `newaliases`
> Postfix ne lit pas `/etc/aliases` directement, mais sa version compilée `/etc/aliases.db`. Après toute modification :
> ```bash
